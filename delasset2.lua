...
local rep, dcrew, junkf, junkn, junkc, pvsmsto, junkl
local coname, coversion, coupdate, coregistered, codbversion
local item, make, modal, oitem, omake, omodal, oserial, oid
 ...
 local function onassetcomplete( event )
   ...
   local function deletedb()
    ...
    local stmt = db:prepare[[DELETE FROM assets WHERE id = ?]];
stmt:bind_values(oid)
stmt:step();
...
--_G.GUI.UnloadThemes("theme_4")
 ...
 local titleText = display.newText( "Delete Office Details - delasset2", halfW, titleBar.y, native.systemFontBold, 22 )
...
mcst:exec( tablesetup )
   ...
   local sql = "SELECT * FROM assets "
     for row in db:nrows(sql) do
        ...
        oserial=row.serial
        oid=row.id
        if seloffice==(offname.."    "..oitem.."    "..oserial) then
           deletedb()
