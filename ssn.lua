...
local halfW = display.contentCenterX
local halfH = display.contentCenterY
local oname, offname, aitem, aid, aasset, aserial
...
local path = system.pathForFile("assets.db", system.DocumentsDirectory)
 -- local path = system.pathForFile("assets.db", system.DocumentsDirectory)
  ...
  --  officear=1
    for row in db:nrows("SELECT * FROM assets order by serial") do
...
aserial=row.serial
      ...
      --  local titleText = display.newText( offname, halfW, titleBar.y, native.systemFontBold, 22 )
         ...
         table.insert(ListData2, newItemCount, {iconL = 20, caption = (aserial.."     "..aitem.."    "..aid)})
end
