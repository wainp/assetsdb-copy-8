...
--_G.GUI.UnloadThemes("theme_4")
 ...
 --local titleText = display.newText( "select office for email", halfW, titleBar.y, native.systemFontBold, 22 )
 --composer:removeScene("scene3")
	...
	onSelect          = function(EventData) 
                                ...
                                -- print("Selected Items Number: "..seled)
                                  seloffice=seled.caption
                                   _G.GUI.RemoveAllWidgets()
                               composer.gotoScene("dofficedetails")
...
function scene:exit( event )
	local group = self.view
        ...
        _G.GUI.GetHandle("Bar1"):show(false)
        _G.GUI.RemoveAllWidgets()
        _G.GUI.GetHandle("offname"):destroy() 
         _G.GUI.GetHandle("listData"):destroy() 
          _G.GUI.GetHandle("lst_main"):destroy() 
         composer.removeScene("doffall")
        composer.purgeScene("doffall")
       -- composer.removeScene("lst_main")
       -- composer.purgeScene("lst_main")
end
